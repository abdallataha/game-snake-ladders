import { RoomState, Player, ClientAction, MoveStep, GameLogEntry, CardColor, GameStatus, GameEvaluation } from '../types';
import { SNAKES, LADDERS, BOARD_SIZE, getNextUniqueAvatarAndColor } from '../utils/board';
import { DEFAULT_CARD_SCHEMES, getPointsForCard, getRoundCardScheme } from '../utils/cards';

// Global room storage to persist across hot reloads and warm serverless lambdas
declare global {
  // eslint-disable-next-line no-var
  var __SL_ROOMS__: Map<string, RoomState> | undefined;
  // eslint-disable-next-line no-var
  var __SL_TIMERS__: Map<string, NodeJS.Timeout> | undefined;
}

export const rooms: Map<string, RoomState> = globalThis.__SL_ROOMS__ || new Map<string, RoomState>();
if (!globalThis.__SL_ROOMS__) {
  globalThis.__SL_ROOMS__ = rooms;
}

export const roomTimers: Map<string, NodeJS.Timeout> = globalThis.__SL_TIMERS__ || new Map<string, NodeJS.Timeout>();
if (!globalThis.__SL_TIMERS__) {
  globalThis.__SL_TIMERS__ = roomTimers;
}

// Broadcast callback hook for WebSockets (if active)
type BroadcastFn = (roomId: string, message: object) => void;
let broadcastCallback: BroadcastFn | null = null;

export function setBroadcastCallback(fn: BroadcastFn | null) {
  broadcastCallback = fn;
}

export function broadcastToRoom(roomId: string, message: object) {
  if (broadcastCallback) {
    broadcastCallback(roomId, message);
  }
}
...
export function calculateMove(from: number, roll: number, exact50: boolean): {
  intermediate: number;
  final: number;
  steps: MoveStep[];
...
export async function processRoundMoves(room: RoomState, isSyncServerless: boolean = false) {
...
export function checkGameOver(room: RoomState) {
...
export function assignRemainingRanks(room: RoomState) {
...
// Lazy Timer Check for Serverless invocations
export function checkLazyRoomTimer(room: RoomState) {
  if (
    room.status === 'playing' &&
    room.roundPhase === 'cards_revealed' &&
    room.timerExpiresAt &&
    Date.now() >= room.timerExpiresAt
  ) {
    // Handle time-out for players who didn't submit
    Object.values(room.players).forEach((p) => {
  ...
  switch (action.type) {
    ...
    case 'JOIN_ROOM': {
      ...
      room.logs.push({
        ...
        intermediate: 0,
        to: 0,
        message: `${newPlayer.name} joined the match!`,
        timestamp: Date.now(),
      });
    ...
    case 'START_GAME': {
      ...
      room.logs.push({
        ...
        intermediate: 0,
        to: 0,
        message: `Match started with ${Object.keys(room.players).length} players! Host reveals cards each round (30s timer).`,
        timestamp: Date.now(),
      });
    ...
    case 'REVEAL_CARDS': {
      ...
      // 30-Second Countdown Timer on Server (active when long-lived process is running)
